...
/*   Updated: 2022/03/30 10:26:24 by eavilov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_map_parser(t_struc *vars, t_assets *content)
{
	...
	check = ft_map_checker(&*vars, &*content);
	if (check == 0)
		return (0);
	vars->mlx = mlx_init();
	vars->win = mlx_new_window(vars->mlx, vars->res[0] * 64,
			vars->res[1] * 64, "C'EST MON SO_LONG");
	ft_wall_placer(&*vars);
