...
/*   Updated: 2022/03/28 15:59:26 by eavilov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_map_checker(char *map, t_struc *vars)
{
	...
	if (ft_isthere_asset(vars->readcont, &*vars) == 0)
	{
		ft_putstr("Error : your map is rectangular but isn't right\n");
		return (0);
	}
	if (ft_is_closed(vars->readcont) == 0)
	{
		ft_putstr("Error : your map has one or multiple holes\n");
		return (0);
	}
	close(i);
	return (1);
